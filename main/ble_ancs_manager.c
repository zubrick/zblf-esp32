...
void ble_ancs_init(void (*_statusCallback)(int), void (*_connectCallback)(uint8_t *), void (*_disconnectCallback)(), char * _btname, uint8_t * bd_addr)
{
  ...
  if (numbound > 0) {
    ...
    esp_ble_bond_dev_t * dev_list = malloc(numbound * sizeof(esp_ble_bond_dev_t));
    ret = esp_ble_get_bond_device_list(&numbound, dev_list);
    if (!ret) {
      for (int i = 0; i < numbound; i++) {
        ESP_LOG_BUFFER_HEX("bound addr", dev_list[i].bd_addr, ESP_BD_ADDR_LEN);
      }
    }
  }
}
